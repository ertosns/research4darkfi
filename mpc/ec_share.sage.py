...
# This file was *autogenerated* from the file ec_share.sage
...
def verify_2pc_mac_check(party0_mac, party1_mac):
    assert party0_mac+party1_mac == _sage_const_0 

global_key = random.randint(_sage_const_0 , p)
...
class ECAuthenticatedShare(object):
      ...
      def __init__(self, share, mac=None, modifier=None):
          ...
          self.public_modifier = _sage_const_0   if modifier == None else modifier # carry out extra addition/subtraction by public scalars until opening

      def copy(self):
          return ECAuthenticatedShare(self.share, self.mac, self.public_modifier)
      ...
      def authenticated_open(self, peer_authenticated_share):
          ...
          mac_key = random.randint(_sage_const_0 ,global_key)
      ...
      def add_point(self, point, party_id):
          return ECAuthenticatedShare(self.share + point, self.mac , self.public_modifier - point) if party_id ==_sage_const_0  else ECAuthenticatedShare(self.share, self.mac, self.public_modifier - point)
...
class ScalingECAuthenticatedShares(object):
      def __init__(self, alpha, beta, triplet, party_id, generator=None, e=None, d=None):
          # authenticated shares
          self.alpha_as = alpha
          self.beta_as = beta
          self.a_as = triplet[_sage_const_0 ]
          self.b_as = triplet[_sage_const_1 ]
          self.c_as = triplet[_sage_const_2 ]
          self.party_id = party_id
          #
          self.generator = generator if generator is not None else CurvePoint.generator()
          d1 = self.alpha_as - self.a_as.mul_point(self.generator)
          e1 = self.beta_as - self.b_as
          self.e = e if e is not None else e1
          self.d = d if d is not None else d1

      def copy(self):
          return ScalingECAuthenticatedShares(self.alpha_as.copy(), self.beta_as.copy(), [self.a_as, self.b_as, self.c_as], self.party_id, self.generator, self.e, self.d)

      def mul(self, d2, e2):
          e = open_2pc(self.e.share, e2.share)
          d = open_2pc(self.d.share, d2.share)
          return (self.b_as.mul_point(d) + self.a_as.mul_point(self.generator).mul_scalar(e) + self.c_as.mul_point(self.generator)).add_point(d * e, self.party_id) if self.party_id ==_sage_const_0  else self.b_as.mul_point(d) + self.a_as.mul_point(self.generator).mul_scalar(e) + self.c_as.mul_point(self.generator)

class MSM(object):
      def __init__(self, points,  scalars, source,  party_id, point_scalars=[]):
          '''
          naive multi scalar multiplicatin, between authenticatedpointsshares, and authenticatedscalarshares
          '''
          self.points = points
          self.scalars = scalars
          assert (len(self.points) == len(self.scalars)), 'len(p): {}, len(s): {}'.format(len(self.points), len(self.scalars))
          self.source = source
          self.party_id = party_id
          beaver = self.source
          self.point_scalars = point_scalars
          if len(self.point_scalars)==_sage_const_0 :
      ...
      def msm(self, de):
          self.point_scalars = [point.mul(de[_sage_const_0 ], de[_sage_const_1 ]) for de, point in zip(de, self.point_scalars)]
          zero_ec_share = ECAuthenticatedShare(_sage_const_0 )
