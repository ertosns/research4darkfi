...
# This file was *autogenerated* from the file ec_msm.sage
...
def sum_shares(shares, source, party_id):
    zero_share = AuthenticatedShare(_sage_const_0 , source, party_id)
    for share in shares:
        zero_share += share
    return zero_share
...
source = Source(p)
points = [CurvePoint.random() for _ in range(_sage_const_0 , N)]
scalars = [random.randint(_sage_const_0 ,p) for i in range(_sage_const_0 , N)]
...
lhs_scalars = [s - random.randint(_sage_const_0 ,p) for s in scalars]
rhs_scalars = [s - lhs for (s, lhs) in zip(scalars, lhs_scalars)]
lhs_scalars_shares = [AuthenticatedShare(s, source, _sage_const_0 ) for s in lhs_scalars]
rhs_scalars_shares = [AuthenticatedShare(s, source, _sage_const_1 ) for s in rhs_scalars]
...
## sas
#print('lhs sas')
lhs_sas_shares = [ScalingECAuthenticatedShares(lhs_points_share, rhs_scalars_share, source.triplet(_sage_const_0 ), _sage_const_0 ) for lhs_points_share, rhs_scalars_share in zip(lhs_points_shares, lhs_scalars_shares)]
#print('rhs sas')
rhs_sas_shares = [ScalingECAuthenticatedShares(rhs_points_share, rhs_scalars_share, source.triplet(_sage_const_1 ), _sage_const_1 ) for rhs_points_share, rhs_scalars_share in zip(rhs_points_shares, rhs_scalars_shares)]
#
...
lhs_sas = [lhs_sas_i.mul(rhs_sas_de_i[_sage_const_0 ], rhs_sas_de_i[_sage_const_1 ]) for lhs_sas_i, rhs_sas_de_i in zip(lhs_sas_shares, rhs_sas_de)]
rhs_sas = [rhs_sas_i.mul(lhs_sas_de_i[_sage_const_0 ], lhs_sas_de_i[_sage_const_1 ]) for rhs_sas_i, lhs_sas_de_i in zip(rhs_sas_shares, lhs_sas_de)]
...
## msm
#print('lhs msm')
lhs_msm = MSM(lhs_points_shares, lhs_scalars_shares, source, _sage_const_0 )
#print('rhs msm')
rhs_msm = MSM(rhs_points_shares, rhs_scalars_shares, source, _sage_const_1 )
